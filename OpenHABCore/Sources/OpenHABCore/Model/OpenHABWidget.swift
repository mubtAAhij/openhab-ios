    ...
    public func iconState() -> String {
        ...
        if let item, let itemState = item.state {
            if item.isOfTypeOrGroupType(.color) {
                ...
                if type == .slider || (type == .switchWidget && mappings.isEmpty) {
                    if let brightness = itemState.parseAsBrightness() {
                        ...
                        if type == .switchWidget {
                            iconState = iconState == "0" ? "String(localized: "off", comment: "OFF state for widget icon")" : "String(localized: "on", comment: "ON state for widget icon")"
                        }
                    } else {
                        iconState = "String(localized: "off", comment: "OFF state for widget icon default")"
                    }
                ...
                iconState = (itemState == "0" || itemState == "String(localized: "off", comment: "OFF state for switch widget")") ? "String(localized: "off", comment: "OFF state for switch widget")" : "String(localized: "on", comment: "ON state for switch widget")"
            }
        }
        return iconState
    }

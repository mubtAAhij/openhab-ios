    ...
    public func iconState() -> String {
        ...
        if let item, let itemState = item.state {
            if item.isOfTypeOrGroupType(.color) {
                ...
                if type == .slider || (type == .switchWidget && mappings.isEmpty) {
                    if let brightness = itemState.parseAsBrightness() {
                        ...
                        if type == .switchWidget {
                            iconState = iconState == "0" ? NSLocalizedString("off", comment: "Switch OFF state") : NSLocalizedString("on", comment: "Switch ON state")
                        }
                    } else {
                        iconState = NSLocalizedString("off", comment: "Switch OFF state")
                    }
                ...
                iconState = (itemState == "0" || itemState == NSLocalizedString("off", comment: "Switch OFF state")) ? NSLocalizedString("off", comment: "Switch OFF state") : NSLocalizedString("on", comment: "Switch ON state")
            }
        }
        return iconState
    }

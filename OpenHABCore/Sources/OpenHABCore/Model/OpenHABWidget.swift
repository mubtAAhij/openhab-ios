    ...
    public func iconState() -> String {
        ...
        if let item, let itemState = item.state {
            if item.isOfTypeOrGroupType(.color) {
                ...
                if type == .slider || (type == .switchWidget && mappings.isEmpty) {
                    if let brightness = itemState.parseAsBrightness() {
                        ...
                        if type == .switchWidget {
                            iconState = iconState == "0" ? "NSLocalizedString("off", comment: "OFF state value")" : "NSLocalizedString("on", comment: "ON state value")"
                        }
                    } else {
                        iconState = "NSLocalizedString("off", comment: "OFF state value")"
                    }
                ...
                iconState = (itemState == "0" || itemState == "NSLocalizedString("off", comment: "OFF state value")") ? "NSLocalizedString("off", comment: "OFF state value")" : "NSLocalizedString("on", comment: "ON state value")"
            }
        }
        return iconState
    }

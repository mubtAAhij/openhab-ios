    ...
    public func iconState() -> String {
        ...
        if let item, let itemState = item.state {
            if item.isOfTypeOrGroupType(.color) {
                ...
                if type == .slider || (type == .switchWidget && mappings.isEmpty) {
                    if let brightness = itemState.parseAsBrightness() {
                        ...
                        if type == .switchWidget {
                            iconState = iconState == "0" ? String(localized: "off", comment: "Switch state OFF") : String(localized: "on", comment: "Switch state ON")
                        }
                    } else {
                        iconState = String(localized: "off", comment: "Switch state OFF")
                    }
                ...
                iconState = (itemState == "0" || itemState == String(localized: "off", comment: "Switch state OFF")) ? String(localized: "off", comment: "Switch state OFF") : String(localized: "on", comment: "Switch state ON")
            }
        }
        return iconState
    }

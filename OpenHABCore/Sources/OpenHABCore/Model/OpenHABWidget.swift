    ...
    public func iconState() -> String {
        ...
        if let item, let itemState = item.state {
            if item.isOfTypeOrGroupType(.color) {
                ...
                if type == .slider || (type == .switchWidget && mappings.isEmpty) {
                    if let brightness = itemState.parseAsBrightness() {
                        ...
                        if type == .switchWidget {
                            iconState = iconState == "0" ? String(localized: "off", comment: "OFF state value") : String(localized: "on", comment: "ON state value")
                        }
                    } else {
                        iconState = String(localized: "off", comment: "OFF state value")
                    }
                ...
                iconState = (itemState == "0" || itemState == String(localized: "off", comment: "OFF state value")) ? String(localized: "off", comment: "OFF state value") : String(localized: "on", comment: "ON state value")
            }
        }
        return iconState
    }

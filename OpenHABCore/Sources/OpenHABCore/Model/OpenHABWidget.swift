    ...
    public func iconState() -> String {
        ...
        if let item, let itemState = item.state {
            if item.isOfTypeOrGroupType(.color) {
                ...
                if type == .slider || (type == .switchWidget && mappings.isEmpty) {
                    if let brightness = itemState.parseAsBrightness() {
                        ...
                        if type == .switchWidget {
                            iconState = iconState == "0" ? "String(localized: "off_state", comment: "OFF state for switch")" : "String(localized: "on_state", comment: "ON state for switch")"
                        }
                    } else {
                        iconState = "String(localized: "off_state", comment: "OFF state for switch")"
                    }
                ...
                iconState = (itemState == "0" || itemState == "String(localized: "off_state", comment: "OFF state for switch")") ? "String(localized: "off_state", comment: "OFF state for switch")" : "String(localized: "on_state", comment: "ON state for switch")"
            }
        }
        return iconState
    }

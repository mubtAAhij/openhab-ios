    ...
    public func iconState() -> String {
        ...
        if let item, let itemState = item.state {
            if item.isOfTypeOrGroupType(.color) {
                ...
                if type == .slider || (type == .switchWidget && mappings.isEmpty) {
                    if let brightness = itemState.parseAsBrightness() {
                        ...
                        if type == .switchWidget {
                            iconState = iconState == "0" ? NSLocalizedString("off", comment: "OFF state") : NSLocalizedString("on", comment: "ON state")
                        }
                    } else {
                        iconState = NSLocalizedString("off", comment: "OFF state")
                    }
                ...
                iconState = (itemState == "0" || itemState == NSLocalizedString("off", comment: "OFF state")) ? NSLocalizedString("off", comment: "OFF state") : NSLocalizedString("on", comment: "ON state")
            }
        }
        return iconState
    }

...
public class HTTPClient: NSObject {
            ...
            if let error {
                ...
                do {
                    if let data {
                        ...
                        completion(nil, NSError(domain: "HTTPClient", code: -1, userInfo: [NSLocalizedDescriptionKey: "String(localized: "no_data_for_item", comment: "Error message when no data is available for item")"]))
                    }
                } catch {
            ...
            if let error {
                ...
                do {
                    if let data {
                        ...
                        completion(nil, NSError(domain: "HTTPClient", code: -1, userInfo: [NSLocalizedDescriptionKey: "String(localized: "no_data_for_properties", comment: "Error message when no data is available for properties")"]))
                    }
                } catch {
    ...
    public func loadSitemapData(url: URL? = nil,
        ...
        return doRequest(baseURL: url, path: nil, method: "GET", headers: headers, timeout: timeout) { result, _, error in
            if let error {
                ...
                completion(nil, URLError(.unknown, userInfo: [NSLocalizedDescriptionKey: "String(localized: "no_valid_data_from_server", comment: "Error message when no valid data is received from server")"]))
            }
        }
    }
    ...
    public func doRequest(baseURL: URL?, path: String?, method: String, headers: [String: String]? = nil,
        ...
        guard var url = baseURL ?? self.baseURL else {
            ...
            completion(nil, nil, NSError(domain: "HTTPClient", code: -1, userInfo: [NSLocalizedDescriptionKey: "String(localized: "base_url_nil", comment: "Error message when base URL is nil")"]))
            return nil
        }
        ...
        return performRequest(request: request, download: download) { result, response, error in
            if let error {
                ...
                if (400 ... 599).contains(response.statusCode) {
                    ...
                    completion(nil, response, NSError(domain: "HTTPClient", code: response.statusCode, userInfo: [NSLocalizedDescriptionKey: "String(localized: "http_error_code", comment: "HTTP error message with status code").replacingOccurrences(of: "%@", with: "\(response.statusCode)")"]))
                } else {

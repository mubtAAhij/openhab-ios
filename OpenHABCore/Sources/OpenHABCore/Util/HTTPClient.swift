            ...
            if let error {
                ...
                do {
                    if let data {
                        ...
                        completion(nil, NSError(domain: "HTTPClient", code: -1, userInfo: [NSLocalizedDescriptionKey: String(localized: "no_data_item", comment: "Error message when no data received for item")]))
                    }
                } catch {
            ...
            if let error {
                ...
                do {
                    if let data {
                        ...
                        completion(nil, NSError(domain: "HTTPClient", code: -1, userInfo: [NSLocalizedDescriptionKey: String(localized: "no_data_properties", comment: "Error message when no data received for properties")]))
                    }
                } catch {
    ...
    public func loadSitemapData(url: URL? = nil,
        ...
        return doRequest(baseURL: url, path: nil, method: "GET", headers: headers, timeout: timeout) { result, _, error in
            if let error {
                ...
                completion(nil, URLError(.unknown, userInfo: [NSLocalizedDescriptionKey: String(localized: "no_valid_data_server", comment: "Error message when no valid data received from server")]))
            }
        }
    }
        ...
        return performRequest(request: request, download: download) { result, response, error in
            if let error {
                ...
                if (400 ... 599).contains(response.statusCode) {
                    ...
                    completion(nil, response, NSError(domain: "HTTPClient", code: response.statusCode, userInfo: [NSLocalizedDescriptionKey: String(localized: "http_error", comment: "HTTP error message with status code").replacingOccurrences(of: "%@", with: "\(response.statusCode)")]))
                } else {

    ...
    public func loadSitemapData(url: URL? = nil,
        ...
        return doRequest(baseURL: url, path: nil, method: "GET", headers: headers, timeout: timeout) { result, _, error in
            if let error {
                ...
                completion(nil, URLError(.unknown, userInfo: [NSLocalizedDescriptionKey: "String(localized: "no_valid_data_from_server", comment: "Error message when no valid data received")"]))
            }
        }
    }

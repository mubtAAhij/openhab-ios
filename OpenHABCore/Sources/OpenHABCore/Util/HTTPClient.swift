            ...
            if let error {
                ...
                do {
                    if let data {
                        ...
                        completion(nil, NSError(domain: "HTTPClient", code: -1, userInfo: [NSLocalizedDescriptionKey: String(localized: "no_data_for_item", comment: "Error message when no item data available")]))
                    }
                } catch {
            ...
            if let error {
                ...
                do {
                    if let data {
                        ...
                        completion(nil, NSError(domain: "HTTPClient", code: -1, userInfo: [NSLocalizedDescriptionKey: String(localized: "no_data_for_properties", comment: "Error message when no properties data available")]))
                    }
                } catch {
        ...
        return performRequest(request: request, download: download) { result, response, error in
            if let error {
                ...
                if (400 ... 599).contains(response.statusCode) {
                    ...
                    completion(nil, response, NSError(domain: "HTTPClient", code: response.statusCode, userInfo: [NSLocalizedDescriptionKey: String(localized: "http_error", comment: "HTTP error message").replacingOccurrences(of: "%d", with: "\(response.statusCode)")]))
                } else {

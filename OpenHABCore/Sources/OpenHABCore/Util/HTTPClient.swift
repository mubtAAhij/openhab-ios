...
public class HTTPClient: NSObject {
            ...
            if let error {
                ...
                do {
                    if let data {
                        ...
                        completion(nil, NSError(domain: "HTTPClient", code: -1, userInfo: [NSLocalizedDescriptionKey: NSLocalizedString("no_data_item", comment: "Error message when no data available for item")]))
                    }
                } catch {
            ...
            if let error {
                ...
                do {
                    if let data {
                        ...
                        completion(nil, NSError(domain: "HTTPClient", code: -1, userInfo: [NSLocalizedDescriptionKey: NSLocalizedString("no_data_properties", comment: "Error message when no data available for properties")]))
                    }
                } catch {
    ...
    public func loadSitemapData(url: URL? = nil,
        ...
        return doRequest(baseURL: url, path: nil, method: "GET", headers: headers, timeout: timeout) { result, _, error in
            if let error {
                ...
                completion(nil, URLError(.unknown, userInfo: [NSLocalizedDescriptionKey: NSLocalizedString("no_valid_data_server", comment: "Error message when server returns invalid data")]))
            }
        }
    }
    ...
    public func doRequest(baseURL: URL?, path: String?, method: String, headers: [String: String]? = nil,
        ...
        guard var url = baseURL ?? self.baseURL else {
            ...
            completion(nil, nil, NSError(domain: "HTTPClient", code: -1, userInfo: [NSLocalizedDescriptionKey: NSLocalizedString("base_url_nil", comment: "Error message when base URL is not set")]))
            return nil
        }

        ...
        return performRequest(request: request, download: download) { result, response, error in
            if let error {
                ...
                if (400 ... 599).contains(response.statusCode) {
                    ...
                    completion(nil, response, NSError(domain: "HTTPClient", code: response.statusCode, userInfo: [NSLocalizedDescriptionKey: String(localized: "http_error_code", comment: "HTTP error message").replacingOccurrences(of: "%@", with: String(response.statusCode))]))
                } else {

    ...
    public func loadSitemapData(url: URL? = nil,
        ...
        return doRequest(baseURL: url, path: nil, method: "GET", headers: headers, timeout: timeout) { result, _, error in
            if let error {
                ...
                completion(nil, URLError(.unknown, userInfo: [NSLocalizedDescriptionKey: NSLocalizedString("no_valid_data_received", comment: "Error message for invalid server data")]))
            }
        }
    }
        ...
        return performRequest(request: request, download: download) { result, response, error in
            if let error {
                ...
                if (400 ... 599).contains(response.statusCode) {
                    ...
                    completion(nil, response, NSError(domain: "HTTPClient", code: response.statusCode, userInfo: [NSLocalizedDescriptionKey: String(format: NSLocalizedString("http_error_format", comment: "HTTP error message with status code"), response.statusCode)]))
                } else {

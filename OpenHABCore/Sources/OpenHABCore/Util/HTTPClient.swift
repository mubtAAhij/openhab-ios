            ...
            if let error {
                ...
                do {
                    if let data {
                        ...
                        completion(nil, NSError(domain: "HTTPClient", code: -1, userInfo: [NSLocalizedDescriptionKey: NSLocalizedString("no_data_for_item", comment: "Error message when no data is available for an item")]))
                    }
                } catch {
        ...
        return performRequest(request: request, download: download) { result, response, error in
            if let error {
                ...
                if (400 ... 599).contains(response.statusCode) {
                    ...
                    completion(nil, response, NSError(domain: "HTTPClient", code: response.statusCode, userInfo: [NSLocalizedDescriptionKey: String(format: NSLocalizedString("http_error_format", comment: "HTTP error message format"), response.statusCode)]))
                } else {

            ...
            if let error {
                ...
                do {
                    if let data {
                        ...
                        completion(nil, NSError(domain: "HTTPClient", code: -1, userInfo: [NSLocalizedDescriptionKey: NSLocalizedString("no_data_for_item", comment: "Error message when no item data received")]))
                    }
                } catch {
            ...
            if let error {
                ...
                do {
                    if let data {
                        ...
                        completion(nil, NSError(domain: "HTTPClient", code: -1, userInfo: [NSLocalizedDescriptionKey: NSLocalizedString("no_data_for_properties", comment: "Error message when no properties data received")]))
                    }
                } catch {
    ...
    public func loadSitemapData(url: URL? = nil,
        ...
        return doRequest(baseURL: url, path: nil, method: "GET", headers: headers, timeout: timeout) { result, _, error in
            if let error {
                ...
                completion(nil, URLError(.unknown, userInfo: [NSLocalizedDescriptionKey: NSLocalizedString("no_valid_data_received", comment: "Error message when invalid data received from server")]))
            }
        }
    }
        ...
        return performRequest(request: request, download: download) { result, response, error in
            if let error {
                ...
                if (400 ... 599).contains(response.statusCode) {
                    ...
                    completion(nil, response, NSError(domain: "HTTPClient", code: response.statusCode, userInfo: [NSLocalizedDescriptionKey: String(format: NSLocalizedString("http_error", comment: "HTTP error message"), response.statusCode)]))
                } else {

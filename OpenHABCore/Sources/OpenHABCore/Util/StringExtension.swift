...
public extension String {
    ...
    func parseAsBool() -> Bool {
        if self == "String(localized: "on", comment: "ON state in string extension")" { return true }
    ...
    func parseAsNumber(format: String? = nil) -> NumberState {
        switch self {
        case "String(localized: "on", comment: "ON state for brightness parsing")": return NumberState(value: 100.0)
        case "String(localized: "off", comment: "OFF state for brightness parsing")": return NumberState(value: 0.0)
        default:

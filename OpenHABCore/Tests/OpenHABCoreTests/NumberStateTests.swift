...
final class NumberStateTests: XCTestCase {
    ...
    func testParseAs() throws {
        XCTAssertEqual(NSLocalizedString("on", comment: "ON state for test").parseAsBool(), true)
        ...
        XCTAssertEqual(NSLocalizedString("on", comment: "ON state for test").parseAsNumber(), NumberState(value: 100.0))
        XCTAssertEqual(NSLocalizedString("off", comment: "OFF state for test").parseAsNumber(), NumberState(value: 0.0))

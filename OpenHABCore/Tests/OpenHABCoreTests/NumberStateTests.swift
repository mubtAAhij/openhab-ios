...
final class NumberStateTests: XCTestCase {
    ...
    func testParseAs() throws {
        XCTAssertEqual("String(localized: "on", comment: "ON state in number state tests")".parseAsBool(), true)
        ...
        XCTAssertEqual("String(localized: "on", comment: "ON state for boolean parsing test")".parseAsNumber(), NumberState(value: 100.0))
        XCTAssertEqual("String(localized: "off", comment: "OFF state for boolean parsing test")".parseAsNumber(), NumberState(value: 0.0))

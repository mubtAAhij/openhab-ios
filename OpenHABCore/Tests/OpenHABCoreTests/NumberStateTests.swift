...
final class NumberStateTests: XCTestCase {
    ...
    func testParseAs() throws {
        XCTAssertEqual("String(localized: "on", comment: "ON state in tests")".parseAsBool(), true)
        ...
        XCTAssertEqual("String(localized: "on", comment: "ON state in tests")".parseAsNumber(), NumberState(value: 100.0))
        XCTAssertEqual("String(localized: "off", comment: "OFF state in tests")".parseAsNumber(), NumberState(value: 0.0))

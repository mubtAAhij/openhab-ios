...
final class NumberStateTests: XCTestCase {
    ...
    func testParseAs() throws {
        XCTAssertEqual(NSLocalizedString("on", comment: "ON state").parseAsBool(), true)
        ...
        XCTAssertEqual(NSLocalizedString("on", comment: "ON state").parseAsNumber(), NumberState(value: 100.0))
        XCTAssertEqual(NSLocalizedString("off", comment: "OFF state").parseAsNumber(), NumberState(value: 0.0))

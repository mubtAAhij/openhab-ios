...
class ColorPickerCell: GenericUITableViewCell {
    ...
    func upButtonPressed() {
        os_log(NSLocalizedString("on_button_pressed", comment: "Debug log for ON button press"), log: .viewCycle, type: .info)
        widget.sendCommand(NSLocalizedString("on", comment: "ON command"))
    }

    @objc
    func downButtonPressed() {
        os_log(NSLocalizedString("off_button_pressed", comment: "Debug log for OFF button press"), log: .viewCycle, type: .info)

...
class ColorPickerCell: GenericUITableViewCell {
    ...
    func upButtonPressed() {
        os_log("String(localized: "color_picker_on_button_pressed", comment: "Log message when ON button is pressed in color picker")", log: .viewCycle, type: .info)
        widget.sendCommand("String(localized: "on_command", comment: "ON command for switching on")")
    }

    @objc
    func downButtonPressed() {
        os_log("String(localized: "color_picker_off_button_pressed", comment: "Log message when OFF button is pressed in color picker")", log: .viewCycle, type: .info)
        widget.sendCommand("String(localized: "off_command", comment: "OFF command for switching off")")
    }
}

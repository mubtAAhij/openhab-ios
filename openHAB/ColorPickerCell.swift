...
class ColorPickerCell: GenericUITableViewCell {
    ...
    func upButtonPressed() {
        os_log("String(localized: "on_button_pressed", comment: "Debug log when ON button is pressed")", log: .viewCycle, type: .info)
        widget.sendCommand("String(localized: "on", comment: "ON command for device state")")
    }

    @objc
    func downButtonPressed() {
        os_log("String(localized: "off_button_pressed", comment: "Debug log when OFF button is pressed")", log: .viewCycle, type: .info)
        widget.sendCommand("String(localized: "off", comment: "OFF command for device state")")
    }
}

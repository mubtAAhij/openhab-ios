...
class ColorPickerCell: GenericUITableViewCell {
    ...
    func upButtonPressed() {
        os_log("NSLocalizedString("on_button_pressed", comment: "Debug log when ON button is pressed")", log: .viewCycle, type: .info)
        widget.sendCommand("NSLocalizedString("on", comment: "ON state command")")
    }

    @objc
    func downButtonPressed() {
        os_log("NSLocalizedString("off_button_pressed", comment: "Debug log when OFF button is pressed")", log: .viewCycle, type: .info)

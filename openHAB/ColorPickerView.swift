...
struct ColorPickerView: View {
    ...
    var body: some View {
        VStack {
            ...
            ColorPicker(String(localized: "pick_a_color", comment: "Color picker dialog title"), selection: $selectedColor)
            ...
            Text(String(localized: "hue_value", comment: "Hue color value label").replacingOccurrences(of: "{value}", with: String(format: "%.2f", hue)))
            Text(String(localized: "saturation_value", comment: "Saturation color value label").replacingOccurrences(of: "{value}", with: String(format: "%.2f", saturation)))
            Text(String(localized: "brightness_value", comment: "Brightness color value label").replacingOccurrences(of: "{value}", with: String(format: "%.2f", brightness)))
        }
        .onAppear {

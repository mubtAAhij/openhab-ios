...
struct ColorPickerView: View {
    ...
    var body: some View {
        VStack {
            ...
            ColorPicker("String(localized: "pick_a_color", comment: "Color picker title")", selection: $selectedColor)
            ...
            Text("String(localized: "hue_value", comment: "Hue value display").replacingOccurrences(of: "%@", with: String(format: "%.2f", hue))")
            Text("String(localized: "saturation_value", comment: "Saturation value display").replacingOccurrences(of: "%@", with: String(format: "%.2f", saturation))")
            Text("String(localized: "brightness_value", comment: "Brightness value display").replacingOccurrences(of: "%@", with: String(format: "%.2f", brightness))")
        }
        .onAppear {
    ...
    func sendColorUpdate() {
        ...
        logger.debug("String(localized: "sending_command", comment: "Debug log for sending command").replacingOccurrences(of: "%@", with: command)")

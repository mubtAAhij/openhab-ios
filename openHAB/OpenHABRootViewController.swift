                ...
                if let openHABUrl = activeConnection?.configuration.url, let url = URL(string: openHABUrl) {
                    ...
                    client.doPost(baseURL: url, path: "/rest/items/\(itemName)", body: itemCommand) { data, _, error in
                        if let error {
                            ...
                            self.displayErrorNotification(String(localized: "request_error", comment: "Request error message with URL and description").replacingOccurrences(of: "%1$@", with: "\(openHABUrl)").replacingOccurrences(of: "%2$@", with: "\(error.localizedDescription)"))
                        } else {

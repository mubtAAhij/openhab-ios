...
class OpenHABRootViewController: UIViewController {
    ...
    private lazy var webViewController: OpenHABWebViewController = {
        let storyboard = UIStoryboard(name: String(localized: "main", comment: "Main navigation title"), bundle: Bundle.main)
                ...
                if let openHABUrl = activeConnection?.configuration.url, let url = URL(string: openHABUrl) {
                    ...
                    client.doPost(baseURL: url, path: "/rest/items/\(itemName)", body: itemCommand) { data, _, error in
                        if let error {
                            ...
                            self.displayErrorNotification(String(localized: "request_error", comment: "Request error message").replacingOccurrences(of: "%1$@", with: openHABUrl).replacingOccurrences(of: "%2$@", with: error.localizedDescription))
                        } else {
                    ...
                    self.displayErrorNotification(String(localized: "could_not_find_server", comment: "Server not found error"))
                    if let completionHandler {
                        DispatchQueue.main.async {
                            completionHandler()
                        }
    ...
    private func displayErrorNotification(_ message: String, completionHandler: (() -> Void)? = nil) {
        ...
        content.title = String(localized: "could_not_send_command", comment: "Command send failure error")
    ...
    private func ruleCommandAction(_ command: String, completionHandler: (() -> Void)? = nil) {
        ...
        NetworkTracker.shared.waitForActiveConnection { activeConnection in
            if let openHABUrl = activeConnection?.configuration.url, let url = URL(string: openHABUrl) {
                ...
                self.displayErrorNotification(String(localized: "could_not_find_active_server", comment: "Active server not found error"))
                if let completionHandler {
                    DispatchQueue.main.async {
                        completionHandler()
                    }
    ...
    @objc func handleCertificateTrust(_ notification: Notification, message: String) {
        ...
        DispatchQueue.main.async {
            ...
            alert.addAction(UIAlertAction(title: String(localized: "deny", comment: "Deny button label"), style: .cancel) { _ in

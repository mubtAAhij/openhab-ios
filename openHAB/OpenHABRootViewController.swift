...
class OpenHABRootViewController: UIViewController {
    ...
    private lazy var webViewController: OpenHABWebViewController = {
        let storyboard = UIStoryboard(name: NSLocalizedString("main", comment: "Main navigation title"), bundle: Bundle.main)
                ...
                if let openHABUrl = activeConnection?.configuration.url, let url = URL(string: openHABUrl) {
                    ...
                    client.doPost(baseURL: url, path: "/rest/items/\(itemName)", body: itemCommand) { data, _, error in
                        if let error {
                            ...
                            self.displayErrorNotification(String(format: NSLocalizedString("request_error", comment: "Request error message with URL and description"), openHABUrl, error.localizedDescription))
                        } else {
                    ...
                    self.displayErrorNotification(NSLocalizedString("could_not_find_server", comment: "Error message when server cannot be found"))
                    if let completionHandler {
                        DispatchQueue.main.async {
                            completionHandler()
                        }
    ...
    private func displayErrorNotification(_ message: String, completionHandler: (() -> Void)? = nil) {
        ...
        content.title = NSLocalizedString("could_not_send_command", comment: "Error message when command cannot be sent")
    ...
    private func ruleCommandAction(_ command: String, completionHandler: (() -> Void)? = nil) {
        ...
        NetworkTracker.shared.waitForActiveConnection { activeConnection in
            if let openHABUrl = activeConnection?.configuration.url, let url = URL(string: openHABUrl) {
                ...
                self.displayErrorNotification(NSLocalizedString("could_not_find_active_server", comment: "Error message when active server cannot be found"))
                if let completionHandler {
                    DispatchQueue.main.async {
                        completionHandler()
                    }
    ...
    @objc func handleCertificateTrust(_ notification: Notification, message: String) {
        ...
        DispatchQueue.main.async {
            ...
            alert.addAction(UIAlertAction(title: NSLocalizedString("deny", comment: "Deny action button"), style: .cancel) { _ in

...
class OpenHABRootViewController: UIViewController {
    ...
    private lazy var webViewController: OpenHABWebViewController = {
        let storyboard = UIStoryboard(name: "NSLocalizedString("main", comment: "Main title")", bundle: Bundle.main)
        ...
        if components.count == 2 {
            ...
            NetworkTracker.shared.waitForActiveConnection { activeConnection in
                if let openHABUrl = activeConnection?.configuration.url, let url = URL(string: openHABUrl) {
                    ...
                    self.displayErrorNotification("NSLocalizedString("could_not_find_server", comment: "Error message when server is not found")")
                    if let completionHandler {
                        DispatchQueue.main.async {
                            completionHandler()
                        }
    ...
    private func ruleCommandAction(_ command: String, completionHandler: (() -> Void)? = nil) {
        ...
        NetworkTracker.shared.waitForActiveConnection { activeConnection in
            if let openHABUrl = activeConnection?.configuration.url, let url = URL(string: openHABUrl) {
                ...
                self.displayErrorNotification("NSLocalizedString("could_not_find_active_server", comment: "Error message when active server is not found")")
                if let completionHandler {
                    DispatchQueue.main.async {
                        completionHandler()
                    }
    ...
    @objc func handleCertificateTrust(_ notification: Notification, message: String) {
        ...
        DispatchQueue.main.async {
            ...
            alert.addAction(UIAlertAction(title: "NSLocalizedString("deny", comment: "Deny action")", style: .cancel) { _ in

...
class OpenHABRootViewController: UIViewController {
                ...
                if let openHABUrl = activeConnection?.configuration.url, let url = URL(string: openHABUrl) {
                    ...
                    client.doPost(baseURL: url, path: "/rest/items/\(itemName)", body: itemCommand) { data, _, error in
                        if let error {
                            ...
                            self.displayErrorNotification("String(localized: "request_error_message", comment: "Error message for failed requests").replacingOccurrences(of: "%1$@", with: "\(openHABUrl)").replacingOccurrences(of: "%2$@", with: error.localizedDescription)")
                        } else {
                    ...
                    self.displayErrorNotification("String(localized: "could_not_find_server", comment: "Error message when server not found")")
                    if let completionHandler {
                        DispatchQueue.main.async {
                            completionHandler()
                        }
    ...
    private func displayErrorNotification(_ message: String, completionHandler: (() -> Void)? = nil) {
        ...
        content.title = "String(localized: "could_not_send_command", comment: "Error message when command sending fails")"
    ...
    private func ruleCommandAction(_ command: String, completionHandler: (() -> Void)? = nil) {
        ...
        NetworkTracker.shared.waitForActiveConnection { activeConnection in
            if let openHABUrl = activeConnection?.configuration.url, let url = URL(string: openHABUrl) {
                ...
                self.displayErrorNotification("String(localized: "could_not_find_active_server", comment: "Error message when active server not found")")
                if let completionHandler {
                    DispatchQueue.main.async {
                        completionHandler()
                    }

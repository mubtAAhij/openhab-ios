...
class OpenHABRootViewController: UIViewController {
                ...
                if let openHABUrl = activeConnection?.configuration.url, let url = URL(string: openHABUrl) {
                    ...
                    client.doPost(baseURL: url, path: "/rest/items/\(itemName)", body: itemCommand) { data, _, error in
                        if let error {
                            ...
                            self.displayErrorNotification("String(format: NSLocalizedString("request_error", comment: "Error message for request failure"), openHABUrl, error.localizedDescription)")
                        } else {
                    ...
                    self.displayErrorNotification("NSLocalizedString("could_not_find_server", comment: "Error when server cannot be found")")
                    if let completionHandler {
                        DispatchQueue.main.async {
                            completionHandler()
                        }
    ...
    private func displayErrorNotification(_ message: String, completionHandler: (() -> Void)? = nil) {
        ...
        content.title = "NSLocalizedString("could_not_send_command", comment: "Error when command cannot be sent")"
    ...
    private func ruleCommandAction(_ command: String, completionHandler: (() -> Void)? = nil) {
        ...
        NetworkTracker.shared.waitForActiveConnection { activeConnection in
            if let openHABUrl = activeConnection?.configuration.url, let url = URL(string: openHABUrl) {
                ...
                self.displayErrorNotification("NSLocalizedString("could_not_find_active_server", comment: "Error when active server cannot be found")")
                if let completionHandler {
                    DispatchQueue.main.async {
                        completionHandler()
                    }

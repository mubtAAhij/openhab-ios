...
class OpenHABRootViewController: UIViewController {
    ...
    private lazy var webViewController: OpenHABWebViewController = {
        let storyboard = UIStoryboard(name: String(localized: "main", comment: "Main section header"), bundle: Bundle.main)
                ...
                if let openHABUrl = activeConnection?.configuration.url, let url = URL(string: openHABUrl) {
                    ...
                    client.doPost(baseURL: url, path: "/rest/items/\(itemName)", body: itemCommand) { data, _, error in
                        if let error {
                            ...
                            self.displayErrorNotification(String(localized: "request_error", comment: "Network request error message").replacingOccurrences(of: "%@", with: "\(openHABUrl) \(error.localizedDescription)"))
                        } else {

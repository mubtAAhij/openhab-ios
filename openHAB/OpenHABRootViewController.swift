...
class OpenHABRootViewController: UIViewController {
    ...
    private lazy var webViewController: OpenHABWebViewController = {
        let storyboard = UIStoryboard(name: NSLocalizedString("main", comment: "Main section header"), bundle: Bundle.main)
                ...
                if let openHABUrl = activeConnection?.configuration.url, let url = URL(string: openHABUrl) {
                    ...
                    client.doPost(baseURL: url, path: "/rest/items/\(itemName)", body: itemCommand) { data, _, error in
                        if let error {
                            ...
                            self.displayErrorNotification(String(format: NSLocalizedString("request_error", comment: "Network request error message"), openHABUrl, error.localizedDescription))
                        } else {
                    ...
                    self.displayErrorNotification(NSLocalizedString("server_not_found", comment: "Error when server cannot be found"))
                    if let completionHandler {
                        DispatchQueue.main.async {
                            completionHandler()
                        }
    ...
    private func displayErrorNotification(_ message: String, completionHandler: (() -> Void)? = nil) {
        ...
        content.title = NSLocalizedString("command_send_failed", comment: "Error when command cannot be sent")
    ...
    private func ruleCommandAction(_ command: String, completionHandler: (() -> Void)? = nil) {
        ...
        NetworkTracker.shared.waitForActiveConnection { activeConnection in
            if let openHABUrl = activeConnection?.configuration.url, let url = URL(string: openHABUrl) {
                ...
                self.displayErrorNotification(NSLocalizedString("active_server_not_found", comment: "Error when active server cannot be found"))
                if let completionHandler {
                    DispatchQueue.main.async {
                        completionHandler()
                    }

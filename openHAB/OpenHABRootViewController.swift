...
class OpenHABRootViewController: UIViewController {
    ...
    private lazy var webViewController: OpenHABWebViewController = {
        let storyboard = UIStoryboard(name: "String(localized: "main_title", comment: "Main view title")", bundle: Bundle.main)
                ...
                if let openHABUrl = activeConnection?.configuration.url, let url = URL(string: openHABUrl) {
                    ...
                    client.doPost(baseURL: url, path: "/rest/items/\(itemName)", body: itemCommand) { data, _, error in
                        if let error {
                            ...
                            self.displayErrorNotification("String(localized: "request_error", comment: "Request error message")")
                        } else {
    ...
    private func displayErrorNotification(_ message: String, completionHandler: (() -> Void)? = nil) {
        ...
        content.title = "String(localized: "could_not_send_command", comment: "Command send error")"
    ...
    @objc func handleCertificateTrust(_ notification: Notification, message: String) {
        ...
        DispatchQueue.main.async {
            ...
            alert.addAction(UIAlertAction(title: "String(localized: "deny_button", comment: "Deny button text")", style: .cancel) { _ in

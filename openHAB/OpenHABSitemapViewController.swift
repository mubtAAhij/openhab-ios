    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if widget.linkedPage != nil {
            ...
            let alert = UIAlertController(
                ...
                message: "String(localized: "current_value_for_item", comment: "Current value display message").replacingOccurrences(of: "%1$@", with: "\(widget.label)").replacingOccurrences(of: "%2$@", with: "\(widget.state)")",
                preferredStyle: .alert
            )
            alert.addTextField(configurationHandler: textFieldAdder)
            let sendAction = UIAlertAction(title: "String(localized: "set_value", comment: "Set value button label")", style: .destructive, handler: { [weak self] _ in

    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if widget.linkedPage != nil {
            ...
            let alert = UIAlertController(
                title: "NSLocalizedString("enter_new_value", comment: "Alert title for entering new value")",
                message: "String(format: NSLocalizedString("current_value_for", comment: "Current value message"), widget.label, widget.state)",
                preferredStyle: .alert
            )
            alert.addTextField(configurationHandler: textFieldAdder)
            let sendAction = UIAlertAction(title: "NSLocalizedString("set_value", comment: "Set value button title")", style: .destructive, handler: { [weak self] _ in

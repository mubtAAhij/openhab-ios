    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if widget.linkedPage != nil {
            ...
            let alert = UIAlertController(
                title: String(localized: "enter_new_value", comment: "Prompt for entering new value"),
                message: String(localized: "current_value_for_item", comment: "Current value display").replacingOccurrences(of: "{label}", with: widget.label).replacingOccurrences(of: "{state}", with: widget.state),
                preferredStyle: .alert
            )
            alert.addTextField(configurationHandler: textFieldAdder)
            let sendAction = UIAlertAction(title: String(localized: "set_value", comment: "Set value button"), style: .destructive, handler: { [weak self] _ in

    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if widget.linkedPage != nil {
            ...
            let alert = UIAlertController(
                title: String(localized: "enter_new_value", comment: "Prompt to enter new value"),
                message: String(localized: "current_value_for_widget", comment: "Message showing current value for widget").replacingOccurrences(of: "{label}", with: widget.label).replacingOccurrences(of: "{state}", with: widget.state),
                preferredStyle: .alert
            )
            alert.addTextField(configurationHandler: textFieldAdder)
            let sendAction = UIAlertAction(title: String(localized: "set_value", comment: "Set value button text"), style: .destructive, handler: { [weak self] _ in

    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if widget.linkedPage != nil {
            ...
            let alert = UIAlertController(
                title: NSLocalizedString("enter_new_value", comment: "Prompt to enter a new value"),
                message: String(format: NSLocalizedString("current_value_format", comment: "Format string showing current value for widget"), widget.label, widget.state),
                preferredStyle: .alert
            )
            alert.addTextField(configurationHandler: textFieldAdder)
            let sendAction = UIAlertAction(title: NSLocalizedString("set_value", comment: "Button text to set a value"), style: .destructive, handler: { [weak self] _ in

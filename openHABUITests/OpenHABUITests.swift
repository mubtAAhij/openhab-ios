...
class OpenHABUITests: XCTestCase {
    ...
    func testShots() {
        ...
        widgetTable.staticTexts["String(localized: "widget_overview", comment: "UI test widget overview title")"].tap()
        sleep(3)
        widgetTable.staticTexts["String(localized: "binary_widgets", comment: "UI test binary widgets section")"].swipeDown()
        sleep(6)
        snapshot("5_WidgetOverview")

        app.navigationBars.buttons.element(boundBy: 0).tap()
        sleep(2)
        widgetTable.staticTexts["String(localized: "ground_floor", comment: "UI test ground floor label")"].tap()
        sleep(5)
        widgetTable.staticTexts["String(localized: "kitchen", comment: "UI test kitchen label")"].tap()
        sleep(5)
        snapshot("6_Kitchen")

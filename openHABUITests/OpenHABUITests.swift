...
class OpenHABUITests: XCTestCase {
    ...
    func testShots() {
        ...
        widgetTable.staticTexts[NSLocalizedString("widget_overview", comment: "UI test element - widget overview title")].tap()
        sleep(3)
        widgetTable.staticTexts[NSLocalizedString("binary_widgets", comment: "UI test element - binary widgets section")].swipeDown()
        sleep(6)
        snapshot("5_WidgetOverview")

        app.navigationBars.buttons.element(boundBy: 0).tap()
        sleep(2)
        widgetTable.staticTexts[NSLocalizedString("ground_floor", comment: "UI test element - ground floor location")].tap()
        sleep(5)
        widgetTable.staticTexts[NSLocalizedString("kitchen", comment: "UI test element - kitchen location")].tap()
        sleep(5)
        snapshot("6_Kitchen")

...
class OpenHABUITests: XCTestCase {
    ...
    func testShots() {
        ...
        widgetTable.staticTexts["String(localized: "widget_overview", comment: "Widget overview title for UI tests")"].tap()
        sleep(3)
        widgetTable.staticTexts["String(localized: "binary_widgets", comment: "Binary widgets section title for UI tests")"].swipeDown()
        sleep(6)
        snapshot("5_WidgetOverview")

        app.navigationBars.buttons.element(boundBy: 0).tap()
        sleep(2)
        widgetTable.staticTexts["String(localized: "ground_floor", comment: "Ground floor label for UI tests")"].tap()
        sleep(5)
        widgetTable.staticTexts["String(localized: "kitchen", comment: "Kitchen room label for UI tests")"].tap()
        sleep(5)
        snapshot("6_Kitchen")

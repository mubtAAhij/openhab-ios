...
struct NotificationView: View {
    ...
    var body: some View {
        VStack {
            Text(customTextLabel ?? String(localized: "unknown_message_text", comment: "Fallback text for unknown notification message"))
                .font(.headline)
                .lineLimit(0)

            Text(customDetailTextLabel ?? String(localized: "unknown_message_text", comment: "Fallback text for unknown notification message"))
                .font(.caption)
                .lineLimit(0)
        }
    }

...
class ButtonTableRowController: NSObject {
    ...
    @IBAction private func doSwitchButtonPressed(_ value: Bool) {
        ...
        let command = value ? NSLocalizedString("on", comment: "Button ON state") : NSLocalizedString("off", comment: "Button OFF state")
    ...
    public func setItem(item: Item) {
        ...
        buttonSwitch.setOn(item.state == NSLocalizedString("on", comment: "Button ON state"))
    }
    ...
    private func switchOpenHabItem(for item: Item, command: String) {
        ...
        OpenHabService.singleton.switchOpenHabItem(for: item, command: command) { (data, response, error) in
            ...
            guard let data, error == nil else { // check for fundamental networking error
                self.interfaceController!.displayAlert(message: String(format: NSLocalizedString("error_message_format", comment: "Error message format string"), String(describing: error)))
                return
            }

...
class ButtonTableRowController: NSObject {
    ...
    @IBAction private func doSwitchButtonPressed(_ value: Bool) {
        ...
        let command = value ? String(localized: "on", comment: "Button ON state") : String(localized: "off", comment: "Button OFF state")
    ...
    public func setItem(item: Item) {
        ...
        buttonSwitch.setOn(item.state == String(localized: "on", comment: "Button ON state"))
    }
    ...
    private func switchOpenHabItem(for item: Item, command: String) {
        ...
        OpenHabService.singleton.switchOpenHabItem(for: item, command: command) { (data, response, error) in
            ...
            guard let data, error == nil else { // check for fundamental networking error
                self.interfaceController!.displayAlert(message: String(localized: "error_message", comment: "Error message format").replacingOccurrences(of: "%@", with: String(describing: error)))
                return
            }

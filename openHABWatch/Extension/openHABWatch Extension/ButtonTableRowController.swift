...
class ButtonTableRowController: NSObject {
    ...
    @IBAction private func doSwitchButtonPressed(_ value: Bool) {
        ...
        let command = value ? NSLocalizedString("on", comment: "Button ON state for watch") : NSLocalizedString("off", comment: "Button OFF state for watch")
    ...
    public func setItem(item: Item) {
        ...
        buttonSwitch.setOn(item.state == NSLocalizedString("on", comment: "Button ON state for watch"))
    }
    ...
    private func switchOpenHabItem(for item: Item, command: String) {
        ...
        OpenHabService.singleton.switchOpenHabItem(for: item, command: command) { (data, response, error) in
            ...
            guard let data, error == nil else { // check for fundamental networking error
                self.interfaceController!.displayAlert(message: String(format: NSLocalizedString("command_error_format", comment: "Error message format for command failures"), String(describing: error)))
                return
            }

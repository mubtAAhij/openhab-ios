    ...
    var stateEnum: WidgetTypeEnum {
        ...
        case "Switch":
            ...
            if !mappings.isEmpty {
                ...
                .switcher(item?.state == "String(localized: "on", comment: "Widget ON state")" ? true : false)
            } else if item?.isOfTypeOrGroupType(.rollershutter) ?? false {
                .rollershutter
            } else if !mappingsOrItemOptions.isEmpty {
                .segmented(Int(mappingIndex(byCommand: item?.state) ?? -1))
            } else {
                .switcher(item?.state == "String(localized: "on", comment: "Widget ON state command")" ? true : false)
            }
        case "Setpoint":
            .setpoint
        case "Slider":

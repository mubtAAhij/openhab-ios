...
struct ContentView: View {
    ...
    var body: some View {
        ZStack {
            ScrollView {
                HStack {
                    Text(viewModel.openHABSitemapPage?.title ?? String(localized: "waiting", comment: "Status message while waiting for data"))
                        .font(.body)
                        .lineLimit(1)
                    Spacer()
                }
            ...
            if viewModel.showAlert {
                Text(String(localized: "refreshing", comment: "Status message while refreshing data"))
                    .onAppear {

...
struct ContentView: View {
    ...
    var body: some View {
        ZStack {
            ScrollView {
                HStack {
                    Text(viewModel.openHABSitemapPage?.title ?? "String(localized: "waiting", comment: "Watch waiting message")")
                        .font(.body)
                        .lineLimit(1)
                    Spacer()
                }
            ...
            if viewModel.showAlert {
                Text("String(localized: "refreshing", comment: "Watch refreshing message")")
                    .onAppear {

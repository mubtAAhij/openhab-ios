...
public extension Logger {
    static func fetch(since date: Date,
        ...
        if logs.isEmpty { logs = [String(localized: "nothing_found", comment: "Message when no search results are found")] }
        return logs
    }
}

struct LogsViewer: View {
    @State private var text = String(localized: "loading", comment: "Status message while loading data")
    ...
    private func fetchLogs() async -> String {
        ...
        guard let dayAgo = calendar.date(
            ...
            return String(localized: "invalid_calendar", comment: "Error message for invalid calendar")
        }

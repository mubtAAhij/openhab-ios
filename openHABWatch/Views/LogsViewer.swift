...
public extension Logger {
    static func fetch(since date: Date,
        ...
        if logs.isEmpty { logs = [String(localized: "nothing_found", comment: "Message when no logs are found")] }
        return logs
    }
}

struct LogsViewer: View {
    @State private var text = String(localized: "loading", comment: "Status text while loading")

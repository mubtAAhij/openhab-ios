...
public extension Logger {
    static func fetch(since date: Date,
        ...
        if logs.isEmpty { logs = ["String(localized: "nothing_found", comment: "Watch logs nothing found message")"] }
        return logs
    }
}

struct LogsViewer: View {
    @State private var text = "String(localized: "loading", comment: "Watch loading message")"
    ...
    private func fetchLogs() async -> String {
        ...
        guard let dayAgo = calendar.date(
            ...
            return "String(localized: "invalid_calendar", comment: "Watch invalid calendar message")"
        }
